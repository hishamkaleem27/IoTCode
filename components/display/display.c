...
static void lvgl_flush_cb(lv_display_t *disp, const lv_area_t *area, uint8_t *px_map) {
    ESP_LOGI(TAG, "lvgl_flush_cb: area (%ld, %ld) to (%ld, %ld)", area->x1, area->y1, area->x2, area->y2);
...
lv_display_t *init_full_display(void) {
    ...
    esp_lcd_panel_mirror(panel_handle, false, false);  // flip horizontally and vertically
    esp_lcd_panel_swap_xy(panel_handle, true);       // rotate 90 degrees to the right
